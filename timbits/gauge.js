...
timbit.about = 'Cyth data retrieval for wild metrix counter data, displays single gauge for a specified server/counter.';
...
timbit.params = {
	...
	counter: {
		description: 'counter to display',
		required: true,
		strict: false,
		values: [ '% Processor Time', 'Requests/sec' ]
	},
	max: {
		description: 'maximum value to be represented, defaults to 100',
		required: true,
		strict: false,
		default: 100,
		values: [ 100, 300 ]
	...
	labelshow: {
		description: 'show the numerical values under the gauge, defaults to off',
		required: true,
		strict: true,
		default: 0,
		values: [ 0, 1 ]
...
timbit.examples = [
	{
		href: '/gauge/?key=abc1234&servername=wpgpdweb83&counter=%25%20Processor%20Time',
		caption: 'Returns Processor Time for WPGPDWEB83, assuming key is valid.'
	}
];
...
timbit.eat = function(req, res, context) {
	...
	client.get( context.index, function( err, val ) {
		...
		for( var row in serverData ) {
			if ( serverData[row].counter.toLowerCase() == context.counter.toLowerCase() ) {
				counter = serverData[row].value;
			}
		}
		...
		if ( undefined == counter ) {
			...
			context.data = counter + ',' + context.max;
		}
		...
		// construct data
		context.header = toTitleCase( context.counter ) + ',total'; // Only the first heading shows on graph, but second needed to indicate second data
